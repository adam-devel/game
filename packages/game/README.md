# The game

Currently the game is tictactoe

# The code

This module defines the game. The purpose of the module to simulate the game.
This module is used by the server to advance the game, and by the client to predict outcomes for smoother gameplay.
This module isn't aware of the server or the client

> [!WARN]
The code here must not rely on any APIs outside besides ECMAScript. It must be portable.

